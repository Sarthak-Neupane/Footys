<template>
    <div class="grid grid-cols-4 grid-rows-4 content-center items-center w-full">
        ...
        <div class="tile innerCell relative " data-grid="[1,1]" data-number="8" ref="OneByOne">
            <div class=" absolute h-full w-full bg-lightWhite"> </div>
        ...
        for (let e of elem) {
            if (e.id === answer.id) {
                ...
                if(answer.playerId === localStorage.getItem('id')) {
                    ...
                    grids[ind].value.firstElementChild.classList.add(`bg-${store.getPlayerColor}`)
                    grids[ind].value.firstElementChild.classList.remove(`bg-lightWhite`)
